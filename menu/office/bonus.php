...
if (!$bonus OR $bonus["last"] + 60 * 60 < time()) { //Kein Eintrag angelegt oder länger her
    ...
    $disabled = "disabled";
    $time = " " . formatSeconds(60 * 60 - time() + $bonus["last"]);
}
...
$output .= "
        ...
        <div>" . put($status, $l) . $time . "</div>
        
        <div class='flexBonus'>
            <div>
                <form action='?page=office&sub=bonus&action=get' method='post'>
                <input type='hidden' name='reward' value='1' />
                <input $disabled type='submit' name='send' value='" . dollar(calcBonus($boni[1])) . "' />
                </form>
            </div>
            <div>
                <form action='?page=office&sub=bonus&action=get' method='post'>
                <input type='hidden' name='reward' value='2' />
                <input $disabled type='submit' name='send' value='" . gas(calcBonus($boni[2])) . "' />
                </form>
            </div>
            <div>
                <form action='?page=office&sub=bonus&action=get' method='post'>
                <input type='hidden' name='reward' value='3' />
                <input $disabled type='submit' name='send' value='" . ep(calcBonus($boni[3])) . "' />
                </form>
            </div>
        </div>
